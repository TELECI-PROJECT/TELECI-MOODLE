...
$lesson_obj = $DB->get_record('lesson', array('id' => $cm->instance), '*', MUST_EXIST);
...
if ($pageid != LESSON_EOL) {
    ...
    //echo ($current_lesson_id+2);
    $url = new moodle_url('/mod/lesson/view.php', array('id' => ($lesson->activitylink), 'forceview' => 1));
    redirect($url, 'Starting next lesson', 5);
    // echo $lessonoutput->header($lesson, $cm, $currenttab, $extraeditbuttons, $lessonpageid, get_string("congratulations", "lesson"));
    // echo $lessoncontent;
    // echo $lessonoutput->footer();
}

$page_data = $DB->get_records_sql('SELECT * FROM {lesson_pages} WHERE id=?', array($pageid));

$page_data = reset($page_data);
//log
$logitem = new stdClass();
$logitem->timestamp = time();
//$logitem->lesson_object = json_encode($lesson_obj);
$logitem->cm_object = json_encode($cm); 
$logitem->page_object = json_encode($page_data); 
if ($page_data->qtype == 20) {
    $logitem->type = 1;
} else if ($page_data->qtype == 3) {
    $logitem->type = 2;
} else {
    $logitem->type = 0;
}
...
$logitem->courseid = $course->id;
$DB->insert_record('lesson_activity_log', $logitem, $returnid=false, $bulk=false);
