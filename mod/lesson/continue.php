...
$lesson_object = $DB->get_record('lesson', array('id' => $cm->instance), '*', MUST_EXIST);
...
$page_data = $DB->get_records_sql('SELECT * FROM {lesson_pages} WHERE id=?', array($page->id));
...
if ($result->answerid > 0) {
        ...
        $logitem->multichoice_result = json_encode($result);
        $logitem->page_object = json_encode($page_data); 
        //$logitem->lesson_object = json_encode($lesson_object); 
